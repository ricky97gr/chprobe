...
var errCodeMap = map[int32]struct {
	...
	ErrRight:              {msgCn: "权限不足", msgEn: "the right is not length"},
	ErrUserDisabled:       {msgCn: "用户已被禁用", msgEn: "user is disabled"},
	ErrUserExist:          {msgCn: "用户名已存在", msgEn: "username already exists"},
}
